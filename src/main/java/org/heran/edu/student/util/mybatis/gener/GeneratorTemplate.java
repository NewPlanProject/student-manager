package org.heran.edu.student.util.mybatis.gener;
...
public class GeneratorTemplate {
	...
	public void generateMapper(String mapperTemplate, File mapperFile) {
		try {
			...
			getTemplate(mapperTemplate).merge(GeneratorContext.getContext(), stringWriter);
	...
	public void generateDomain(String className, String domainTemplate, File domainFile) {
		try {
			...
			StringWriter stringWriter = new StringWriter();
			GeneratorContext.setParameter(serialVersionUID, null);
			getTemplate(domainTemplate).merge(GeneratorContext.getContext(), stringWriter);

			GeneratorContext.setParameter(serialVersionUID,
			...
			getTemplate(domainTemplate).merge(GeneratorContext.getContext(), writer);
	...
	public void generateDao(String daoTemplate, File daoFile) {
		try {
			StringWriter stringWriter = new StringWriter();
			getTemplate(daoTemplate).merge(GeneratorContext.getContext(), stringWriter);
			String content = formatMapper(stringWriter.toString());
	...
	private long serialVersionUID(String className, String sourceCode) {
		try {
			String packageName = GeneratorContext.getDomainPackage();
			...
			String buildPath = GeneratorContext.getBuildPath();
